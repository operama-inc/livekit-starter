#!/usr/bin/env python3
"""
Unified Agent V3 - With transcript and audio recording
Single worker that handles both customer and support roles and saves conversations
"""
import os
import logging
import json
...
# Output directory for conversations
OUTPUT_DIR = Path("data/livekit_conversations")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


class RecordingAgent(Agent):
    """Agent that records conversations"""

    def __init__(self, role: str, persona, room_name: str):
        self.role = role
    ...
    def save_transcript(self):
        """Save the conversation transcript to file"""
        timestamp = self.start_time.strftime("%Y%m%d_%H%M%S")
        filename = OUTPUT_DIR / f"{self.room_name}_{timestamp}_transcript.json"
...
async def recording_entrypoint(ctx: JobContext):
    """Entry point that records conversations"""

    job_id = ctx.job.id if hasattr(ctx, 'job') else "unknown"
    room_name = ctx.room.name

    logger.info("="*50)
    logger.info(f"RECORDING AGENT: Received job request")
    ...
    logger.info(f"Connected as {role} agent - RECORDING ENABLED")
    ...
    # Monitor session and save transcript when done
    try:
        # Keep session alive
        while True:
            await asyncio.sleep(1)
    except asyncio.CancelledError:
        # Save transcript when shutting down
        transcript_file = agent.save_transcript()
        logger.info(f"Conversation ended. Transcript saved to: {transcript_file}")
        raise
...
if __name__ == "__main__":
    ...
    cli.run_app(WorkerOptions(
        entrypoint_fnc=recording_entrypoint
